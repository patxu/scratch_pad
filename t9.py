...
class Solution():
    ...
    def test(self):
        inputs = [
                ([2, 2, 8], ["act","bat","cat","acd","test"], [["act"], ["bat"], ["cat"]]), 
                ([7, 6, 6, 3, 8, 4, 6, 3], ["some","time","rome","sometime","so","me"], [["rome","time"],["so","me","time"],["some","time"],["sometime"]])
                  ]
    ...
    def solution(self, input_digits, valid_words):
        t9_perm = [self.t9[digit] for digit in input_digits]
            
        results = []
        self.helper(t9_perm, '', [], set(valid_words), results)
    ...
    # 228
    # ['a', 'b', 'c'], ['a', 'b', 'c'], ['t', 'u', 'v']
    def helper(self, t9_perm, candidate, words_so_far, valid_words, results):
        if candidate in valid_words:
            if len(t9_perm) == 0:
                results.append(words_so_far + [candidate])
            else:
                self.helper(t9_perm, '', words_so_far + [candidate], valid_words, results)

        for char in next(iter(t9_perm), []):
            self.helper(t9_perm[1:], candidate + char, words_so_far[:], valid_words, results)
